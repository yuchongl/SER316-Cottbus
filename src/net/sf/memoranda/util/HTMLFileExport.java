    ...
    private void doExport() {
	 ...
	 private String applyTemplate() {
        ...
        try {
            ...
            //String urlParamaters = "title" + URLEncoder.encode(note.getTitle(), "UTF-8") + "content"+sw.toString();
            String urlParamaters = "mem?op=notes_op&action=save&title=" + URLEncoder.encode("HardcodedValue", "UTF-8") + "&content=" + URLEncoder.encode(body, "UTF-8");
            System.out.println(title);
            URL url = new URL("http://104.236.193.225:7501/" + urlParamaters);
            connection = (HttpURLConnection) url.openConnection();
            connection.setRequestMethod("POST");
            connection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");

            connection.setRequestProperty("Content-Length", Integer.toString(urlParamaters.getBytes().length));
